...
int main()
{
	string name="ThusSpokeZarathustra";srand((unsigned int)time(0));
...
//	system(("g++ "+name+".cpp -o "+name).c_str());
	rep(id,1,6)
	{
		...
		int n=200000,k=2000,rnd=0;
		if(!sample)
		{
			if(id<=2) n=20,k=1;
			else if(id<=4) n=200,k=3;
			else if(id<=6) n=2000,k=40;
			else if(id<=8) n=200000,rnd=1;
			else n=200000,k=2000;
		}
		else{
			if(id==1) n=10,k=1;
			else if(id==2) n=20,k=1;
			else if(id==3) n=200,k=3;
			else if(id==4) n=2000,k=5;
			else if(id==5) n=200000,rnd=1;
			else n=200000,k=200;
		}
		rep(i,1,n) printf("%c",char(rnd?getn('a','z'):(getn(0,k)==0?'b':'a')));printf("\n");
		rep(i,1,n) printf("%c",char(rnd?getn('a','z'):(getn(0,k)==0?'b':'a')));printf("\n");
