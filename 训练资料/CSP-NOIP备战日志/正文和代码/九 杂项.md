...
显然式子等价于$a^2+b^2 \equiv x(\bmod p)$，既然我们需要求的是a，可以把式子继续变为$a^2 \equiv x-b^2(\bmod p)$

对于一个数a，a是否满足$a^2 \equiv x(\bmod p)$是可以预处理出来的，这也就意味着对于每一个题目中的x，一个数字是否合法可以很快地判断出来。
