...
我们用每一个点的点权表示，显然入度为0的点点权为1。 当我们拓扑排序删边的时候，可以用转移前的点权+1来更新转移后的点，之后输出每个点的点权即可。 
...
如果火车经过车站但是没有停下，说明分级必然小于通过的站点，用建边的方式维护这种关系，然后求一个拓扑序就可以了。同时进入队列的点数据同一个分级，只要记录`whlie()`循环中有入度归零的次数即可。
